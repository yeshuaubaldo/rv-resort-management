    ...
    <div className="min-h-screen bg-slate-100 p-8 font-sans relative">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
          ...
          <button onClick={handleLogout} className="bg-white text-red-600 border border-red-100 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all shadow-sm">
            Sign Out
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-4 mb-8">
          ...
          <div className="flex bg-white p-1 rounded-3xl shadow-sm border border-slate-200 overflow-x-auto">
            {/* ADDED 'Rejected' TO THE ARRAY BELOW */}
            {['Pending', 'Confirmed', 'Rejected', 'Checked Out'].map((s) => (
              <button
                key={s}
                onClick={() => setFilter(s)}
                className={`px-6 py-3 rounded-2xl font-black text-[10px] whitespace-nowrap uppercase tracking-widest transition-all ${filter === s ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'text-slate-400 hover:text-slate-600'}`}
              >
                {s}
              </button>
            ))}
              ...
              <div key={booking.id} className="bg-white rounded-[3rem] p-8 shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6 hover:shadow-xl hover:scale-[1.01] transition-all duration-300">
                ...
                <div className="text-center md:text-left px-10 border-x border-slate-50 w-full md:w-auto flex flex-col gap-2">
                  ...
                  <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                    <p className="text-[8px] font-black text-slate-500 uppercase">GCash Ref: <span className="text-blue-600 ml-1">{booking.payment_ref || 'NONE'}</span></p>
                    {booking.payment_image_url && (
                      <button onClick={() => window.open(getImageUrl(booking.payment_image_url), '_blank')} className="mt-2 w-full bg-blue-600 text-white text-[8px] font-black py-2 rounded-lg hover:bg-blue-700 transition-all uppercase">
                        View Receipt
                      </button>
                    )}
                  </div>
                ...
                <div className="flex gap-2 w-full md:w-auto">
                  {booking.status === 'Pending' && (
                    <>
                      <button
                        ...
                        className="flex-1 bg-green-600 text-white px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-green-700 transition-all shadow-lg shadow-green-100"
                      >
                        Approve
                      </button>
                      {/* REJECT BUTTON ADDED HERE */}
                      <button
                        onClick={() => updateStatus(booking.id, 'Rejected')}
                        className="flex-1 bg-red-100 text-red-600 px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all"
                      >
                        Reject
                      </button>
                    </>
                  ...
                  {/* Option to move Rejected back to Pending if mistake */}
                  {booking.status === 'Rejected' && (
                    <button
                      onClick={() => updateStatus(booking.id, 'Pending')}
                      className="flex-1 bg-slate-200 text-slate-600 px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-300 transition-all"
                    >
                      Re-review
                    </button>
                  )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] p-10 max-w-sm w-full shadow-2xl">
            <div className="text-center">
              ...
              <p className="text-slate-500 font-bold text-xs uppercase tracking-tight leading-relaxed mb-8">This action cannot be undone.</p>
              <div className="flex flex-col gap-3">
                <button onClick={confirmDelete} className="w-full bg-red-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-red-700">Delete</button>
                <button onClick={() => setDeleteModal({ open: false, id: null })} className="w-full bg-slate-100 text-slate-500 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest">Cancel</button>
              </div>
            </div>
          </div>
        </div>
